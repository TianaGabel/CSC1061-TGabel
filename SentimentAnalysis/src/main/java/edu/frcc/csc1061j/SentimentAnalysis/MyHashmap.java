...
public class MyHashmap<K, V> implements Map<K, V> {
	...
	public V put(K key, V value) {
		...
		size++;
		//if the number of entries is more than half the length of the array, we rehash
		if ((size*1.0)/buckets.length > loadFactorThreshold) {
			rehash();
		}
		return value;
	}
	...
	// Homework 2
	private void rehash() {
		//I decided this would be a little more straight forward than repeating code again.
		//gets list of entries
		Set<java.util.Map.Entry<K, V>> pairs = entrySet();
		//Doubles the size of the array
		buckets = new LinkedList[buckets.length*2];
		//iterates over keys to add them to the new map
		if (pairs != null) {
			for(java.util.Map.Entry<K, V> entry: pairs) {
